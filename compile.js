...
while (args.length > 0) {
	...
	switch (v) {
		...
		break;
	    default:
		if (~filenames.indexOf(v)) break;
		filenames.push(v);
		if (forceCommonJS) moduleFiles[v] = true;
		break;
	}
}
...
function loadModule(moduleId, currentModuleId, pathIndex) {
	...
	fs.readFile(file, "utf8", function(err, code) {
		if (err) {
			if (++pathIndex == paths.length) throw new Error('No module "' + moduleId + '" exists in paths [' + paths.join(', ') + '], required in ' + currentModuleId + '.js');
			--loading;
		...
		combined.push('require.addModuleClosure("' + moduleId + '", function(require, exports, module, undefined) {\n' + code + '\n});\n\n');
		...
		if (!--loading) {
			if (main && !modules[main]) {
				...
				if (main) {
					if (afterHtml) combined.push('$(function() { require.run("' + main + '"); });');
					else combined.push('require.run("' + main + '");');
				}
				finish(combined);
			}
		}
...
function loadFile(file) {
	...
	fs.readFile(file, "utf8", function(err, code) {
		...
		if ((moduleFiles[file] && !exempt) || (commonjs && !exempt && hasRequireOrExports(code))) {
			var moduleId;
			if (paths.length) {
			...
			if (moduleId) moduleId = moduleId.replace(/\.js$/, '');
			findRequired(code, moduleId);
			if (moduleId) code = 'require.addModuleClosure("' + moduleId + '", function(require, exports, module, undefined) {\n' + code + '\n});';
		}
...
function findRequired(code, moduleId) {
	...
	code = code.replace(/\/\/.*|\/\*(.|\s)*?\*\//g, ''); // remove line comments and block comments
